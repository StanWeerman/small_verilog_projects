...
// DESIGN
	...
	fifo_np fifo_np(.clk(sys_clk), .reset(Reset), .data_in(sw), .wen(write), .ren(read), .data_out(data_out), .depth(depth), .empty(LED16_R), .full(LED17_R), .fill_array(LED));
